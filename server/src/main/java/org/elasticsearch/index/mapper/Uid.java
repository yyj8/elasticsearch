...
public final class Uid {
     ...
     *  what UTF8 would do.
     *  对索引中存储的id进行编码。这种编码针对数字id和base64 id进行了优化，它们的编码方式比UTF8要密集得多。
     *  @see #decodeId */
    public static BytesRef encodeId(String id) {
        if (id.isEmpty()) {
            throw new IllegalArgumentException("Ids can't be empty");
        }
        if (isPositiveNumeric(id)) {//判断是否是正数，遍历字符串中每个字符
            // common for ids that come from databases with auto-increments
            return encodeNumericId(id);//编码数值类型ID
        } else if (isURLBase64WithoutPadding(id)) {
            // common since it applies to autogenerated ids
            return encodeBase64Id(id);//编码base64类型ID
        } else {
            return encodeUtf8Id(id);//编码除数字、base64两种类型以外的其他字符串类型
        }
    }

    public static void main(String[] args) {
        System.out.println(encodeId("a1002").toString());;
    }
